//Use Raspberry Pi Pico to interface with BNO-055 and motors for the EW309 Turret Spring FY24 Semester
// J. Bradshaw 20240104
// Check for older BNO-055 Orientation with the connector 
...
#define FIRE_PIN  D14 // belt driver motor - WHITE
#define SPIN_PIN  D15 // spinner motors for launching ball - YELLOW
...
// Initialize Motor Port Objects (ME1 to ME1, ME2 to ME2 on motor driver board cables)
// Motor Port on J2 is Pitch control
int motPitch_EN = D8;   // ME2 on J2 - GP8 when gun barrel points down, negative pitch angle on IMU and positive
                        // corrective output
int motPitch_IN1 = D9;  // IN2 on J2 - GP9 on Pico
int motPitch_IN2 = D10; // IN1 on J2 - GP10 on Pico

// Motor Port on J1 is Yaw Control
int motYaw_EN = D11;    // ME1 on J1 - GP11 when gun barrel goes positive Yaw (down looking clockwise), a negative motor corrective output
int motYaw_IN1 = D12;   // IN2 on J1 - GP12 on Pico 
int motYaw_IN2 = D13;   // IN1 on J1 - GP13 on Pico
...
// SPIN motor variables
#define MAX_SPIN_TIME 3.0
int spin_flag = 0;
float spin_time = 0.0;  // time in seconds to SPIN motor on (max is MAX_SPIN_TIME)
unsigned long spinTimeLast;
...
//        pitchMeas = -(PI - yawMeas) - PI;        
  ...
  if(Serial.available()){
      if(Serial.peek() == '?'){ // ? display commands
        ...
        delay(10);
        Serial.printf("spin x.x\\r - turn on spin motor for x.x seconds\r\n");
...
//            tempf = -(PI - tempf) - PI;
      ...
      // spin motor drive on for time X (max 3 sec)
      if(strncmp(str, "spin ", 5) == 0){ 
...
//        digitalWrite(SPIN_PIN, LOW);   
        ...
        if(numvals == 1){                
          if(timef < 0.0)
            timef = 0.0;
          if(timef > MAX_SPIN_TIME)
            timef = MAX_SPIN_TIME;

          spin_time = (unsigned long)(timef * 1000.0) + millis(); // update the global variable for feedtime
          spin_flag = 1;
          Serial.printf("\r\nSpin T=%7.3f\r\n", timef);
        }
        else{
          Serial.printf("\r\nBad spin command received\r\n");
        }
      }// motor spin command
  ...
  // if the spin and fire command is triggered
  if(firing == 1){
    //Serial.printf("Entered firing!\r\n");
    if(fireNumTimes > 0){
      digitalWrite(SPIN_PIN, HIGH);
      delay(500);
      if(millis() > (spinTimeLast + FIRE_TIME_DELAY)){
